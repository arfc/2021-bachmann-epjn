...
archetype \cite{huff_fundamental_2016}\DIFaddbegin \DIFadd{, which determines if additional 
facilities are required to meet the specified demand}\DIFaddend . The \glspl{LWR} are deployed 
...
recognized as part of the }\Cycamore \DIFadd{GrowthRegion and how each facility contributes 
...
\DIFaddbegin \DIFadd{After 2025, an average of 0.302$\times 10^6$ kg-\mbox{%DIFAUXCMD
\gls{SWU} }\hspace{0pt}%DIFAUXCMD
is required to 
enrich the uranium sent to the \mbox{%DIFAUXCMD
\glspl{LWR}}\hspace{0pt}%DIFAUXCMD
.
...
An average of \DIFdelbegin \DIFdel{48.50 }\DIFdelend \DIFaddbegin \DIFadd{73.12 }\DIFaddend MTU/month and a maximum of 719.25 
...
\DIFaddbegin \DIFadd{A total of 81,747.7 MTU are sent to all of the reactors after 2025 and 
...
a higher enrichment level. An average of \DIFdelbegin \DIFdel{1.37}\DIFdelend \DIFaddbegin \DIFadd{2.07}\DIFaddend $\times 10^6$ kg-\gls{SWU}/month
...
\glspl{LWR} in Scenario 1. \DIFaddbegin \DIFadd{A total of 16.8$\times 10^8$ kg-SWU and 14.4$\times 
...
requires a larger \mbox{%DIFAUXCMD
\gls{SWU} }\hspace{0pt}%DIFAUXCMD
capacity than enriching uranium for the 
all \mbox{%DIFAUXCMD
\glspl{LWR} }\hspace{0pt}%DIFAUXCMD
in the scenario despite the larger mass of enriched uranium required 
by the \mbox{%DIFAUXCMD
\glspl{LWR} }\hspace{0pt}%DIFAUXCMD
because the \mbox{%DIFAUXCMD
\glspl{MMR} }\hspace{0pt}%DIFAUXCMD
require a higher enrichment level. 
}\DIFaddend 
...
lower than in Scenario 1. An average of \DIFdelbegin \DIFdel{23.81 
...
\DIFaddbegin \DIFadd{A total of 112,453.6 MTU and 81,818.6 MTU are required for all of the 
...
prior to 2025. \DIFaddbegin \DIFadd{A total of 17.4$\times 10^8$ kg-SWU and 15.1$\times 10^8$
kg-SWU are required to enrich uranium for all reactors after 2025 and the advanced 
...
time step when the \mbox{%DIFAUXCMD
\gls{MMR} }\hspace{0pt}%DIFAUXCMD
is deployed in Scenarios 2 and 4. For the time 
steps at which the \mbox{%DIFAUXCMD
\glspl{MMR} }\hspace{0pt}%DIFAUXCMD
receive fuel (134, 181 occurences), the difference present 
...
in Scenarios 2 and 4 in which uranium is not sent to the \mbox{%DIFAUXCMD
\glspl{MMR}}\hspace{0pt}%DIFAUXCMD
. At these time 
...
require periods of significantly more \mbox{%DIFAUXCMD
\gls{SWU} }\hspace{0pt}%DIFAUXCMD
capacity. The \mbox{%DIFAUXCMD
\gls{SWU} }\hspace{0pt}%DIFAUXCMD
capacity 
...
of shipment to the reactors or starting with non-natural uranium. An example of a 
possible facility for enriching uranium for the \mbox{%DIFAUXCMD
\glspl{MMR} }\hspace{0pt}%DIFAUXCMD
in Scenario 2 would 
have a monthly throughput of , and a \mbox{%DIFAUXCMD
\gls{SWU} }\hspace{0pt}%DIFAUXCMD
capacity of  
}
...
\gls{HALEU} }\hspace{0pt}%DIFAUXCMD
required by each of the transitions. This }\DIFdelend \DIFaddbegin \DIFadd{material requirements if the \mbox{%DIFAUXCMD
\gls{HALEU} }\hspace{0pt}%DIFAUXCMD
...
below 5\% $^{235}$U. Finally, an investigation into strategies 
to meet uranium mass demand of the reactors while reducing the peaks in 
the required \mbox{%DIFAUXCMD
\gls{SWU} }\hspace{0pt}%DIFAUXCMD
capacity }\DIFaddend will provide insight into \DIFdelbegin \DIFdel{how this \mbox{%DIFAUXCMD
\gls{HALEU}
}\hspace{0pt}%DIFAUXCMD
production method can be used to meet the material requirements of these 
transition scenarios}\DIFdelend \DIFaddbegin \DIFadd{the logistics 
of deploying a transition to the advanced reactors}\DIFaddend .
