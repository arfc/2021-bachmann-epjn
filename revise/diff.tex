...
%DIF ADD revisions.tex   Tue Sep 21 08:30:39 2021
...
\end{keyword}
\begin{abstract}
\DIFdelbegin \DIFdel{There are two primary ways to produce }%DIFDELCMD < \gls{HALEU} 
%DIFDELCMD < %%%
\DIFdel{for advanced
...
material requirements of the transition to \DIFdelbegin %DIFDELCMD < \gls{HALEU}%%%
\DIFdel{-fueled }\DIFdelend \DIFaddbegin \DIFadd{High Assay Low Enriched 
...
\DIFdel{and X-energy Xe-100 were chosen to represent the microreactor 
...
However, 
deploying the }%DIFDELCMD < \gls{MMR} %%%
\DIFdel{causes periods of large increases of required 
}\DIFdelend \DIFaddbegin \DIFadd{before the transition. Fueling Xe-100 reactors requires less 
}\DIFaddend enriched uranium and \DIFdelbegin %DIFDELCMD < \gls{SWU} %%%
\DIFdel{capacity that are not present when 
...
two primary methods for creating }%DIFDELCMD < \gls{HALEU}%%%
\DIFdel{: }%DIFDELCMD < \gls{HEU} %%%
\DIFdel{recovery and 
down-blending, 
and the enrichment of }%DIFDELCMD < \gls{NU}%%%
\DIFdel{. Downblending }%DIFDELCMD < \gls{HEU} %%%
\DIFdel{is 
limited by the physical inventory of }%DIFDELCMD < \gls{HEU} %%%
\DIFdel{available. 
Enriching }%DIFDELCMD < \gls{NU} %%%
\DIFdel{is limited by the amount }\DIFdelend \DIFaddbegin \DIFadd{The mass }\DIFaddend of uranium that can be mined and the \gls{SWU} capacity \DIFdelbegin \DIFdel{to enrich the uranium}\DIFdelend \DIFaddbegin \DIFadd{available 
...
to obtain the }%DIFDELCMD < \gls{HALEU} %%%
\DIFdel{fuel needed }\DIFdelend \DIFaddbegin \DIFadd{inform the material 
...
data about the currently deployed }%DIFDELCMD < \glspl{LWR}%%%
\DIFdel{. This database provides }\DIFdelend \DIFaddbegin \DIFadd{provided }\DIFaddend the 
grid connection date and power level of each \DIFdelbegin \DIFdel{reactor deployed}\DIFdelend \DIFaddbegin \gls{LWR}\DIFaddend , and the 
...
level }\DIFdelend \DIFaddbegin \DIFadd{for any }\gls{LWR} \DIFadd{still in 
...
\DIFadd{Knowing a reactor's }\gls{EOL} \DIFadd{burnup, power output, and cycle length
allows for calculating its initial load of fissile materials.
For the }\gls{MMR}\DIFadd{, considering a burnup of 42.7 MWd/kgU 
\mbox{%DIFAUXCMD
...
\DIFadd{The mass of uranium for the fresh }\gls{TRISO} \DIFadd{pebbles for the Xe-100 
reactor was found by calculating the total volume of UCO }\gls{TRISO} 
\DIFadd{particle kernel in a fuel pebble and the number of 
...
disposal in the }\gls{HLW} \DIFaddend Sink. Figure 
...
\DIFdelbegin \DIFdel{Material compositions are defined using recipes. Spent and fresh }%DIFDELCMD < \gls{LWR} 
%DIFDELCMD < %%%
\DIFdel{fuel recipes were obtained from \mbox{%DIFAUXCMD
\cite{yacout_visionverifiable_2006}}\hspace{0pt}%DIFAUXCMD
. The spent }\DIFdelend \DIFaddbegin \DIFadd{Recipes define the composition of the materials shown in Figure 
\ref{fig:fuel_cycle}. Yacout et al. \mbox{%DIFAUXCMD
\cite{yacout_visionverifiable_2006}
}\hspace{0pt}%DIFAUXCMD
supplied recipes for spent and fresh }\DIFaddend \gls{LWR} fuel
\DIFdelbegin \DIFdel{recipe assumes }\DIFdelend \DIFaddbegin \DIFadd{assuming }\DIFaddend a 51 MWd/kg-U burnup. All other recipes capture the 
...
The enrichment facility assumes the feed material is }\gls{NU}\DIFaddend , 
...
of 30,635.0 MTU are sent to }\glspl{LWR} \DIFadd{after 2025 in this scenario. 
}\DIFaddend 
...
sent to the }\glspl{LWR}\DIFadd{.
After 2025, an average of 0.302$\times 10^6$ kg-}\gls{SWU} \DIFaddend is required to 
...
to enrich uranium sent to }\glspl{LWR} \DIFadd{after 2025. 
}\DIFaddend 
...
9,182. \DIFaddbegin \DIFadd{This is about an order of magnitude greater than the }\glspl{LWR}
\DIFadd{deployed in 2025.
}\DIFaddend 
...
GWe-y, before the \DIFaddbegin \Cycamore \DIFadd{ManagerInst deploys }\DIFaddend \glspl{MMR}\DIFdelbegin \DIFdel{are 
deployed}\DIFdelend .
...
causes the }\DIFaddend increases in uranium sent to \DIFdelbegin \DIFdel{these reactors }\DIFdelend \DIFaddbegin \glspl{MMR} \DIFaddend and the periods of 
...
The }\glspl{MMR} \DIFadd{receive an average of 73.12 }\DIFaddend MTU/month and a maximum of 719.25 
MTU \DIFdelbegin \DIFdel{are sent to the }%DIFDELCMD < \glspl{MMR} %%%
...
which no }%DIFDELCMD < \gls{HALEU} %%%
\DIFdel{is sent to }\DIFdelend \DIFaddbegin \DIFadd{steps when additional 
}\glspl{MMR} \DIFadd{are not deployed. The average mass of uranium required to fuel 
}\DIFaddend the \glspl{MMR} \DIFdelbegin \DIFdel{due to the lack of refueling}\DIFdelend \DIFaddbegin \DIFadd{is more than what is required by }\gls{EG} \DIFadd{02 
\mbox{%DIFAUXCMD
...
}\glspl{MMR} \DIFadd{receive a total of 51,112.7 MTU. These totals show that most 
...
just the \glspl{MMR}. \DIFdelbegin \DIFdel{There is a large difference in the }\DIFdelend \DIFaddbegin \DIFadd{Enriching uranium for the }\glspl{MMR} \DIFadd{requires a 
greater }\DIFaddend \gls{SWU} capacity \DIFdelbegin \DIFdel{needed to enrich uranium for }\DIFdelend \DIFaddbegin \DIFadd{than enriching uranium for the }\DIFaddend \glspl{LWR} 
\DIFdelbegin \DIFdel{and }%DIFDELCMD < \glspl{MMR}%%%
\DIFdel{. This 
is }\DIFdelend \DIFaddbegin \DIFadd{prior to 2025 }\DIFaddend because the \glspl{MMR} require \DIFdelbegin \DIFdel{a much larger mass and 
}\DIFdelend \DIFaddbegin \DIFadd{uranium at }\DIFaddend a higher enrichment 
level. \DIFdelbegin \DIFdel{An average of 
1.37}\DIFdelend \DIFaddbegin \DIFadd{Enriching uranium for the }\glspl{MMR} \DIFadd{requires an average of 
2.07}\DIFaddend $\times 10^6$ kg-\gls{SWU}/month and a maximum of 20.3$\times 10^6$ 
kg-\gls{SWU}\DIFdelbegin \DIFdel{are needed to enrich the uranium sent to }%DIFDELCMD < \glspl{MMR}%%%
...
just the }\glspl{MMR} \DIFadd{requires 14.4$\times 10^8$ kg-SWU. 
}\DIFaddend 
...
reactors. \DIFaddbegin \DIFadd{Nine times more }\glspl{MMR} \DIFadd{are needed than Xe-100 reactors to 
...
while it is sent to the }%DIFDELCMD < \glspl{LWR} %%%
\DIFdel{every 18 months. }\DIFdelend An average of 
...
than the fuel mass required by the }\glspl{HTGR} \DIFadd{in }\gls{EG} \DIFadd{02 
\mbox{%DIFAUXCMD
\cite{wigeland_nuclear_2014}}\hspace{0pt}%DIFAUXCMD
...
due to the 
multiple time steps in which no uranium is sent to the }%DIFDELCMD < \glspl{MMR}%%%
\DIFdelend \DIFaddbegin \DIFadd{. 
...
Scenario 2. The average \DIFdelbegin \DIFdel{is slightly higher }\DIFdelend \DIFaddbegin \gls{SWU} \DIFadd{capacity to enrich uranium for 
the }\glspl{MMR} \DIFadd{is slightly greater }\DIFaddend than the average \gls{SWU} 
...
fewer than the }\glspl{MMR} \DIFadd{required to meet the same energy demand. 
}\DIFaddend 
...
\DIFadd{prior to 2025
at each time step. Fueling the }\glspl{MMR} \DIFadd{in Scenarios 2 and 4 requires the 
most uranium at any single time step, but the average uranium mass is less 
than what is required to fuel the }\glspl{LWR} \DIFadd{prior to 2025. This is because 
of multiple time steps in which no or a small mass (less than 20 MTU) 
of uranium is sent to }\glspl{MMR}\DIFadd{, which offset the timesteps that require 
a large mass of uranium (more than 200 MTU) are sent to the }\glspl{MMR}\DIFadd{. 
}
...
\DIFadd{requires more uranium than deploying the Xe-100 reactor for the same 
transition scenario. Deploying the }\gls{MMR} \DIFadd{in a no growth transition 
...
\DIFadd{Figure \ref{fig:swu_all} compares the }\gls{SWU} \DIFadd{capacity required to enrich uranium for 
all reactors and the advanced reactors in each scenario. The }\gls{SWU} \DIFadd{capacity required 
to enrich uranium in Scenarios 1, 3, and 5 are similar in magnitude. Scenarios 2 and 4
appear to require a greater }\gls{SWU} \DIFadd{capacity because the calculated }\gls{SWU} \DIFadd{capacity 
is based on the mass of uranium sent to the reactors at each time step. Therefore, 
the values presented do not reflect the required }\gls{SWU} \DIFadd{capacity of 
...
\begin{figure}
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \caption{\DIFaddFL{Total }\gls{SWU} \DIFaddFL{required to enrich uranium sent to all reactors at each time step.}}
    ...
    \DIFaddFL{\hspace{0.8cm}
        ...
        \caption{\gls{SWU} \DIFaddFL{required to enrich uranium sent to advanced reactors at each time step.}}
        \label{fig:haleuswu_al}
    \end{subfigure}
    \caption{\gls{SWU} \DIFaddFL{required to enrich natural uranium in all scenarios.}}
    \label{fig:swu_all}
\end{figure}

\DIFadd{An example of a possible facility for enriching uranium for the }\glspl{MMR} \DIFadd{in 
Scenario 2 would 
have a throughput of 66 MTU/month, and a }\gls{SWU} \DIFadd{capacity of 2.28$\times 10^6$
...
This theoretical example shows how a constant-capacity }\gls{HALEU} \DIFadd{enrichment 
...
reactor from the current fleet of \DIFaddbegin \DIFadd{U.S. }\DIFaddend \glspl{LWR}\DIFdelbegin \DIFdel{in the US. The }\DIFdelend \DIFaddbegin \DIFadd{. We used the }\DIFaddend current 
...
\DIFaddbegin \DIFadd{More }\glspl{MMR} \DIFadd{are required than Xe-100 reactors to meet the same 
...
the mass of enriched uranium and required }\DIFdelend \DIFaddbegin \DIFadd{requires a smaller average 
uranium mass but a greater }\DIFaddend \gls{SWU} capacity \DIFdelbegin \DIFdel{, because }\DIFdelend \DIFaddbegin \DIFadd{than fueling }\glspl{LWR} \DIFadd{prior 
to 2025 because the }\gls{MMR} \DIFadd{requires a higher enrichment level. 
Fueling }\DIFaddend \glspl{MMR} \DIFdelbegin \DIFdel{require a large mass of enriched uranium when they are 
first deployed and do not require refueling. This behavior is not observed 
in the transitions to }\DIFdelend \DIFaddbegin \DIFadd{involves large, one-time shipments of fuel, while 
...
\DIFdelbegin \DIFdel{amount of }%DIFDELCMD < \gls{HEU} %%%
\DIFdel{required to produce }\DIFdelend \DIFaddbegin \DIFadd{material requirements if }\DIFaddend the \gls{HALEU} \DIFdelbegin \DIFdel{required by each of the transitions. This will provide insight into how this }\DIFdelend \DIFaddbegin \DIFadd{demand is met by downblending 
}\gls{HEU} \DIFadd{or enriching uranium of non-natural enrichment, such as 
}\gls{LEU} \DIFadd{below 5\% $^{235}$U. Finally, an investigation into the 
optimization of an enrichment facility or the use of other methods to 
create }\DIFaddend \gls{HALEU} \DIFdelbegin \DIFdel{production method can be used }\DIFdelend to meet the \DIFdelbegin \DIFdel{material requirements of these 
transition scenarios}\DIFdelend \DIFaddbegin \DIFadd{demand of each scenario will provide 
insight into the logistics of transitioning to }\gls{HALEU}\DIFadd{-fueled 
reactors}\DIFaddend . 
