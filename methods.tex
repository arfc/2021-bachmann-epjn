...
required for each refueling, was obtained from supplementary soources 
...
archetype \cite{huff_fundamental_2016}. The \glspl{LWR} are depoyed 
...
The simulations model the fuel cycle from the uranium mine to the final 
...
2-5, and the facilities in red are the advanced reactors, 
which use \gls{HALEU} fuel. Although the back end of the fuel 
...
Material compositions are defined using recipes. The spent \gls{LWR} fuel 
recipe assumes a 
...
Enrichment feed recipes assume natural uranium is used, and the tails assay 
is 0.2\%. 